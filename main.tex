...
\email{lukas.eder@thi.de}
\orcid{0009-0008-1579-3855}
\author{Jakob Löw}
\authornotemark[1]
...
% In order for the described attacks to work, the attacker needs physical access to the charging station and the vehicle.
...
% In our initial tests we built a man-in-the-middle box featuring a CCS socket and a CCS plug, allowing to plug in the charging station into one side and plug the connector into the vehicle on the other side.
% Figure \ref{fig:supercharger} shows this initial setup in action, performing a man-in-the-middle attack on a charging session of our test vehicle.


\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{graphics/tesla-supercharger-mitm.jpg}
    \caption{Man-in-the-middle setup with adapter box to tap the CP and the PE pins}
    \label{fig:supercharger}
    \Description{Picture of the hardware setup for sniffing the charging communication between the Supercharger and the Tesla}
\end{figure}
...
% While the box simply forwards all connections from the socket to the plug, the components used inside come with a maximum current limit based on temperature limitations.
% In order to prove the feasibility of our attack as well as circumvent limits set by the box, later we moved away from actual connections towards inductive coupling of the communication lines.
...
%While professional inductive coupling devices, which can be mounted around cables exist, our tests show that simply twisting the sniffer modem communication wires around the charging cables is enough to get a low enough signal to noise ratio, allowing to tap into the charging communication.
...
\begin{figure}[ht]
    ...
    \label{fig:inductive}
    \Description{Picture of the hardware setup for sniffing the charging communication with inductive coupling}
\end{figure}
